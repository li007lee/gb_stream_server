...
// s=<session name>
// There MUST be one and only one "s=" field per session description. can be empty
static int sdp_parse_y(struct sdp_t* sdp)
{
	size_t n = 0;
...
const char* sdp_y_get(struct sdp_t* sdp)
{
	return sdp->y;
}
...
char* sdp_media_type(struct sdp_t* sdp, int media)
{
	struct sdp_media *m;
...
char* sdp_media_attribute_find(struct sdp_t* sdp, int media, const char* name)
{
	size_t i;
	struct sdp_media *m;
...
int rtsp_get_session_uri(void *sdp, char* uri, int bytes, const char* requri, const char* baseuri, const char* location)
{
	char path[256] = {0};
	const char* control;
